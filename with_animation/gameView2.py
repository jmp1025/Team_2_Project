...
class GameView(arcade.View):
    ...
    def on_draw(self):
        ...
        if self.in_diolouge:
            arcade.draw_texture_rectangle(
                1000, 300, 400, 500, self.diolouge_box_texture)
            arcade.draw_text(
                f'{self.diolouge.multiplier} x {self.level}', 1000, 475, arcade.color.BLACK, 35)
        ...
        arcade.draw_text(f'Lives: {self.lives}', 10,
                         680, arcade.color.DARK_RED, 35)

        if self.near_npc != None and self.in_diolouge != True:
            arcade.draw_text('Press E to interact with NPC',
                             400, 600, arcade.color.BLACK, 24)
        ...
        for x in range(map.size[0]):
            for y in range(map.size[1]):
                if pix[x, y] == (255, 255, 255, 255):
                    ...
                    self.objective = arcade.Sprite(
                        "Images/Game_Goal.png", scale=0.3)
    ...
    def move_frame(self):
        ...
        if self.player.center_x > 500:
            ...
            for background in self.list_of_backgrounds:
                background.center_x += (abs(self.player.center_x - 300)
                                        * math.sqrt(3)) / 4
            for spriteList in self.list_of_sprite_lists:
                for sprite in spriteList.sprite_list:
                    sprite.center_x += abs(self.player.center_x -
                                           300) * math.sqrt(3)
...
class StartMenuView(arcade.View):
    ...
    def on_key_release(self, key, mods):
        if key == arcade.key.L:
            user_input = arcade.gui.UIInputBox(
                SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2, 400, 300)
            user_input.render()
